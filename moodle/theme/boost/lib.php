...
function theme_boost_before_standard_top_of_body_html()
{
    global $PAGE, $USER;
    ...
    // Inject general JS
    $PAGE->requires->js(new moodle_url('/theme/boost/javascript/post.js'), true);

    // Inject student-only CSS
    if ($isStudent) {
        $PAGE->requires->css(new moodle_url('/theme/boost/style/student.css'));
        $PAGE->requires->js(new moodle_url('/theme/boost/javascript/student.js'), true);
    }
}
